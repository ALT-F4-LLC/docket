...
func NewBrowser(conn *sql.DB, projectID int, docketDir string) tea.Model {
...
func (m browserModel) Init() tea.Cmd {
	return loadViewCmd(m.conn, m.projectID, m.view, m.viewRequestID)
}
...
func loadViewCmd(conn *sql.DB, projectID int, view viewMode, requestID int) tea.Cmd {
	debugLogf("load view cmd: view=%s request=%d", view, requestID)
	switch view {
	case viewBoard:
		return loadBoardCmd(conn, projectID, requestID)
	default:
		return loadListCmd(conn, projectID, requestID)
	}
}

func loadListCmd(conn *sql.DB, projectID, requestID int) tea.Cmd {
	return func() tea.Msg {
		debugLogf("running list query: request=%d", requestID)
		data, err := app.ListIssues(conn, app.ListIssuesParams{ProjectID: projectID, Limit: defaultListLimit})
		return listLoadedMsg{view: viewList, requestID: requestID, data: data, err: err}
	}
}

func loadBoardCmd(conn *sql.DB, projectID, requestID int) tea.Cmd {
	return func() tea.Msg {
		debugLogf("running board query: request=%d", requestID)
		data, err := app.LoadBoard(conn, app.BoardParams{ProjectID: projectID})
