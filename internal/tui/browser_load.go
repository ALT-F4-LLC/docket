...
func (m *browserModel) beginViewLoad() tea.Cmd {
	...
	return loadViewCmd(m.conn, m.view, m.listSort, m.viewRequestID, m.projectID)
}
...
func loadViewCmd(conn *sql.DB, view viewMode, listSort sortMode, requestID int, projectIDs ...int) tea.Cmd {
	debugEventf("refresh_started", "view=%s sort_field=%s sort_dir=%s request_id=%d", view, listSort.Field, listSort.Dir, requestID)
	switch view {
	case viewBoard:
		return loadBoardCmd(conn, requestID, projectIDs...)
	default:
		return loadListCmd(conn, listSort, requestID, projectIDs...)
	}
}

func loadListCmd(conn *sql.DB, listSort sortMode, requestID int, projectIDs ...int) tea.Cmd {
	return func() tea.Msg {
...
func loadBoardCmd(conn *sql.DB, requestID int, projectIDs ...int) tea.Cmd {
	return func() tea.Msg {
		...
		data, err := loadBoard(conn, app.BoardParams{ProjectID: projectID})
