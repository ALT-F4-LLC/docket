...
func TestLinkDocIssue(t *testing.T) {
	...
	testsupport.Must(t, err, "GetDocIssues: %v", err)
...
func TestGetIssueDocs(t *testing.T) {
	...
	err := LinkDocIssue(db, docA, issueID)
	testsupport.Must(t, err, "LinkDocIssue A: %v", err)
	err = LinkDocIssue(db, docB, issueID)
	testsupport.Must(t, err, "LinkDocIssue B: %v", err)

	ids, err := GetIssueDocs(db, issueID)
	testsupport.Must(t, err, "GetIssueDocs: %v", err)
	if len(ids) != 2 {
...
func TestLinkDocIssue_DuplicateReturnsErrConflict(t *testing.T) {
	...
	err := LinkDocIssue(db, docID, issueID)
	testsupport.Must(t, err, "first LinkDocIssue: %v", err)
	err = LinkDocIssue(db, docID, issueID)
...
func TestUnlinkDocIssue(t *testing.T) {
	...
	testsupport.Must(t, err, "UnlinkDocIssue: %v", err)
...
func TestHydrateDocs_BatchNoNPlus1(t *testing.T) {
	...
	err := LinkDocIssue(db, docB, id1)
	testsupport.Must(t, err, "LinkDocIssue docB→id1: %v", err)
	err = LinkDocIssue(db, docA, id1)
	testsupport.Must(t, err, "LinkDocIssue docA→id1: %v", err)
	err = LinkDocIssue(db, docC, id1)
	testsupport.Must(t, err, "LinkDocIssue docC→id1: %v", err)
	err = LinkDocIssue(db, docB, id2)
	testsupport.Must(t, err, "LinkDocIssue docB→id2: %v", err)

	issues := []*model.Issue{{ID: id1}, {ID: id2}, {ID: id3}}
	err = HydrateDocs(db, issues)
	testsupport.Must(t, err, "HydrateDocs: %v", err)
...
func TestHydrateDocs_Empty(t *testing.T) {
	...
	err := HydrateDocs(db, nil)
	testsupport.Must(t, err, "HydrateDocs nil: %v", err)
	err = HydrateDocs(db, []*model.Issue{})
	testsupport.Must(t, err, "HydrateDocs empty: %v", err)
}
...
func TestHydrateLinkedIssues_BatchNoNPlus1(t *testing.T) {
	...
	err := LinkDocIssue(db, docA, id2)
	testsupport.Must(t, err, "LinkDocIssue docA→id2: %v", err)
	err = LinkDocIssue(db, docA, id1)
	testsupport.Must(t, err, "LinkDocIssue docA→id1: %v", err)
	err = LinkDocIssue(db, docB, id3)
	testsupport.Must(t, err, "LinkDocIssue docB→id3: %v", err)

	refs, err := HydrateLinkedIssues(db, []int{docA, docB})
	testsupport.Must(t, err, "HydrateLinkedIssues: %v", err)
...
func TestHydrateLinkedIssues_Empty(t *testing.T) {
	...
	testsupport.Must(t, err, "HydrateLinkedIssues nil: %v", err)
	if len(refs) != 0 {
	...
	testsupport.Must(t, err, "HydrateLinkedIssues unlinked: %v", err)
	if len(refs[docID]) != 0 {
...
func TestGetProposalDocs(t *testing.T) {
	...
	err := LinkProposalDoc(db, pID, docA)
	testsupport.Must(t, err, "LinkProposalDoc A: %v", err)
	err = LinkProposalDoc(db, pID, docB)
	testsupport.Must(t, err, "LinkProposalDoc B: %v", err)
...
func TestLinkProposalDoc_DuplicateReturnsErrConflict(t *testing.T) {
	...
	err := LinkProposalDoc(db, pID, docID)
	testsupport.Must(t, err, "first LinkProposalDoc: %v", err)
	err = LinkProposalDoc(db, pID, docID)
	if !errors.Is(err, ErrConflict) {
...
func TestUnlinkProposalDoc(t *testing.T) {
	...
	testsupport.Must(t, err, "UnlinkProposalDoc: %v", err)
...
func TestInsertProposalDocLink_RoundTrip(t *testing.T) {
	...
	testsupport.Must(t, err, "InsertProposalDocLink: %v", err)
	if !inserted {
		t.Error("inserted = false, want true")
	}
