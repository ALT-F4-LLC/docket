...
func configureUIOutputForTest(t *testing.T) {
	t.Helper()
...
func TestRenderUIListFooterBarWrapsInsteadOfTruncatingAtNarrowWidths(t *testing.T) {
	...
	for _, fragment := range []string{"2 board", "refresh auto 5s not loaded", "s sort-field", "S sort-dir", "J/K detail", "o drill-down", "q quit"} {
		if !strings.Contains(rendered, fragment) {
