...
func runIssueShow(cmd *cobra.Command, args []string, w *output.Writer) error {
	...
	result := showResult{
		...
		LinkedProposals: data.LinkedProposals,
	...
	var message string
	if !w.JSONMode {
		message = render.RenderDetail(data.Issue, data.SubIssues, data.Relations, data.LinkedProposals, data.Comments, data.Activity)
	}
