...
test_w_link() {
  ...
  run create --json -t "Link Target"
  assert_exit "W" "W0_tgt" 0
  local LINK_TGT
  LINK_TGT=$(extract_id)

  run create --json -t "Link Third"
  assert_exit "W" "W0_third" 0
  local LINK_THIRD
  LINK_THIRD=$(extract_id)

  run create --json -t "Link Fourth"
  assert_exit "W" "W0_fourth" 0
  local LINK_FOURTH
  LINK_FOURTH=$(extract_id)

  # W1: docket link basic (JSON) — create a blocks relation
  run link "$LINK_SRC" blocks "$LINK_TGT" --json
  assert_exit "W" "W1" 0
  ...
  run link "$LINK_TGT" blocks "$LINK_SRC" --json
  assert_exit "W" "W7" 4

  # W8: Cycle detection for blocks (exit 4)
  # SRC blocks TGT from W1. Create TGT blocks FOURTH, then FOURTH blocks SRC.
  run link "$LINK_TGT" blocks "$LINK_FOURTH" --json
  assert_exit "W" "W8_setup" 0
  run link "$LINK_FOURTH" blocks "$LINK_SRC" --json
  assert_exit "W" "W8" 4

  # W9: Cycle detection for depends_on (exit 4)
  # THIRD depends_on SRC from W3. Trying SRC depends_on THIRD creates a cycle.
  run link "$LINK_SRC" depends-on "$LINK_THIRD" --json
  assert_exit "W" "W9" 4

  # W10: DKT-prefix accepted
  run link "DKT-$LINK_SRC" duplicates "DKT-$LINK_FOURTH" --json
  assert_exit "W" "W10" 0

  # W11: Invalid relation type (exit 3)
  run link "$LINK_SRC" invalid-type "$LINK_TGT" --json
  assert_exit "W" "W11" 3

  # W12: Link to non-existent issue (exit 2)
  run link "$LINK_SRC" blocks 9999 --json
  assert_exit "W" "W12" 2

  # W13: Invalid issue ID (exit 3)
  run link "abc" blocks "$LINK_TGT" --json
  assert_exit "W" "W13" 3

  # W14: docket links (JSON) — shows all relations
  run links "$LINK_SRC" --json
  assert_exit "W" "W14" 0
  ...
  run create --json -t "Isolated Issue"
  assert_exit "W" "W18_setup" 0
  local ISOLATED_ID
  ISOLATED_ID=$(extract_id)
  run links "$ISOLATED_ID" --json
  assert_exit "W" "W18" 0
  ...
  # W19: docket unlink (JSON) — remove a relation
  # Remove the duplicates relation created in W10
  run unlink "$LINK_SRC" duplicates "$LINK_FOURTH" --json
  assert_exit "W" "W19" 0
  ...
  run link "$LINK_SRC" duplicates "$LINK_FOURTH" --json
  assert_exit "W" "W20_setup" 0
  run unlink "$LINK_SRC" duplicates "$LINK_FOURTH"
  assert_exit "W" "W20" 0
