...
test_x_next() {
  ...
  local HAS_DONE
  HAS_DONE=$(echo "$CMD_STDOUT" | jq "[.data.issues[] | select(.id == \"DKT-$NEXT_DONE\")] | length" 2>/dev/null)
  check_cond "X" "X2_no_done" "done issue DKT-$NEXT_DONE should not appear in next" [ "$HAS_DONE" -eq 0 ]

  # X3: Blocked issues are excluded (NEXT_BLOCKED is blocked by NEXT_BLOCKER which is not done).
  local HAS_BLOCKED
  HAS_BLOCKED=$(echo "$CMD_STDOUT" | jq "[.data.issues[] | select(.id == \"DKT-$NEXT_BLOCKED\")] | length" 2>/dev/null)
  check_cond "X" "X3_no_blocked" "blocked issue DKT-$NEXT_BLOCKED should not appear in next" [ "$HAS_BLOCKED" -eq 0 ]

  # X4: Parent issues with children are excluded (not leaf tasks).
  local HAS_PARENT
  HAS_PARENT=$(echo "$CMD_STDOUT" | jq "[.data.issues[] | select(.id == \"DKT-$NEXT_PARENT\")] | length" 2>/dev/null)
  check_cond "X" "X4_no_parent" "parent issue DKT-$NEXT_PARENT should not appear in next (not a leaf)" [ "$HAS_PARENT" -eq 0 ]

  # X5: Independent leaf issues DO appear (NEXT_IND, NEXT_BLOCKER, NEXT_CHILD).
  local HAS_IND
  HAS_IND=$(echo "$CMD_STDOUT" | jq "[.data.issues[] | select(.id == \"DKT-$NEXT_IND\")] | length" 2>/dev/null)
  check_cond "X" "X5_has_ind" "independent issue DKT-$NEXT_IND should appear in next" [ "$HAS_IND" -ge 1 ]
  ...
  check_cond "X" "X7_status" "expected only todo issues when filtering by status" [ "$TODO_ONLY" -eq 0 ]
  ...
  check_cond "X" "X8_priority" "expected only critical issues when filtering by priority" [ "$CRIT_ONLY" -eq 0 ]
  ...
  assert_exit "X" "X14" 0
  local NOW_READY
  NOW_READY=$(echo "$CMD_STDOUT" | jq "[.data.issues[] | select(.id == \"DKT-$NEXT_BLOCKED\")] | length" 2>/dev/null)
  check_cond "X" "X14_unblocked" "issue DKT-$NEXT_BLOCKED should be ready after blocker was closed" [ "$NOW_READY" -ge 1 ]
}
